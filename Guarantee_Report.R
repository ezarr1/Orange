...
#----------------------------------------------------------------------------------------------------------
total_gbv_with_Guarantee_type <- Loans %>% mutate(type.guarantee = ifelse(id.loan %in% Guarantee_Raw$loan.id,Guarantee_Raw$guarantee.type,FALSE)) %>% 
                                    filter(type.guarantee != 'Without') %>% summarise(s = sum(gbv.original))
gbv_with_Guarantee_type <- Loans %>% mutate(type.guarantee = ifelse(id.loan %in% Guarantee_Raw$loan.id,Guarantee_Raw$guarantee.type,FALSE)) %>% 
                                           filter(type.guarantee != 'Without') %>% 
                                           group_by(type.guarantee) %>%    
                                           summarise(Perc = sum(gbv.original)/total_gbv_with_Guarantee_type$s)
...
total_gbv_with_Guarantee_type <- Loans %>%
  inner_join(Guarantee_Raw, by = c("id.loan" = "loan.id")) %>%
  summarise(s = sum(gbv.original))

gbv_with_Guarantee_type <- Loans %>%
  inner_join(Guarantee_Raw, by = c("id.loan" = "loan.id")) %>%
  group_by(guarantee.type) %>%
  summarise(Perc = sum(gbv.original) / total_gbv_with_Guarantee_type$s)
